...
- **Point-in-time docs are disposable.** QA reports, gap reviews, and implementation specs get this footer when created: `*Disposable point-in-time doc. Delete once fully actioned (see AGENTS.md).*` Delete them once actioned. Living docs (runbooks, SOPs, CLAUDE.md, AGENTS.md) stay.
