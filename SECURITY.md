...
| Cross-app localStorage leak | Medium | **Mitigated** — the app should be hosted on its own dedicated subdomain (e.g. `kujira-investments.github.io`) so its localStorage is isolated from any other app on the same parent domain. |
